...
def get_market_brain(candles):
    ...
    if macro_bias=="bull":
        ...
        if bear_votes==3 and at_spike:
            ...
            conviction=62; opt_conviction="atm"  # raised: puts lose more in bull market
        elif bear_votes>=2:
            conviction=38; opt_conviction="atm"  # need stronger signal for puts
        else:
...
def api_manual_opt():
    ...
    r=b.api.order("buy",1,pid)
    if r.get("success"):
        b.opts.open(symbol)
        b.emit("TRADE",f"MANUAL OPT {opt_type.upper()} {symbol} ${premium:.2f}")
        b.trades.append({"time":datetime.now(timezone.utc).isoformat(),"side":opt_type,
            "entry":round(mark,4),"exit":None,"lots":1,"pnl":None,"pct":None,
