...
class C:
    ...
    OPT_TP=0.70; OPT_FLOOR=0.15; OPT_FLOOR_TRAIL=0.25; OPT_STOP=0.30
...
def api_positions(): return jsonify({"perp":bot._pos_display(),"options":bot._opts_display()})
...
def api_close_all():
    ...
    for p in bot.api.btcusd_pos():
        if bot.api.close(float(p.get("size",0)),p.get("product_id",C.PID)).get("success"): n+=1
    for p in bot.api.opt_pos():
        if bot.api.close(float(p.get("size",0)),p.get("product_id")).get("success"): n+=1
...
def api_manual():
    ...
    p=bot.price or bot.api.price(); lots=max(1,int(d.get("lots",1)))
...
def api_opts_toggle():
    d=request.json or {}; bot.opts_mode=bool(d.get("enabled",not bot.opts_mode))
    bot.emit("INFO","Options ON" if bot.opts_mode else "Options OFF")
...
def api_opts_find():
    ...
    d=request.json or {}; t=d.get("type","call"); p=bot.price or bot.api.price()
...
def api_opts_straddle():
    if not bot.opts_eng: return jsonify({"error":"Not connected"})
    return jsonify(bot.opts_eng.straddle(bot.price or bot.api.price()))
...
def api_debug():
    ...
    bal,_,err=bot.api.balance(); out["balance"]=bal; out["err"]=err
    return jsonify(out)
