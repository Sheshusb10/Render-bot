...
class C:
    ...
    PID    = 27; SYMBOL="BTCUSD"; LOT=0.001; LEV=5; SCAN=300  # Delta contract: 1 lot = 0.001 BTC
...
class Bot:
    def __init__(self):
        ...
        self.lot_size=10         # lots per trade (10 lots = 0.01 BTC per trade)
    ...
    def scan(self):
        ...
        # Margin per lot: price * contract_size / leverage
        # 1 lot = 0.001 BTC. At $77000 BTC, 5x leverage: $77000*0.001/5 = $15.40 per lot
        margin_per_lot=self.price*C.LOT/C.LEV
        # Use user's lot_size setting, but cap by available capital (max 10% per trade)
        max_affordable=max(1,int(self.capital*0.10/margin_per_lot))
        lots=min(self.lot_size, max_affordable)
        if lots<1: lots=1
        total_margin=lots*margin_per_lot
        self.emit("INFO",f"Trade: {lots}L = {lots*C.LOT:.3f} BTC | margin ${total_margin:.2f} | cap ${self.capital:.2f}")
    ...
    def state(self):
        ...
        return {
            ...
            "user_settings":{"lot_size":self.lot_size,"max_daily":self.max_daily,"daily_trades":self._daily_trades,"lot_btc":round(self.lot_size*C.LOT,4),"lot_usd":round(self.lot_size*C.LOT*(self.price or 77000),2)},
            "guardrails":{
