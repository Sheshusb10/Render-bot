...
def get_market_brain(candles):
    ...
    # Options need different signals: IV environment, time value, trend strength
    opt_score = 0
    opt_pillars = {}

    # Pillar 1: Trend direction (40pts) — most important for options
    if direction=="long" and macro_bias=="bull":
        ...
        opt_pillars["Trend"] = {"s":0,"m":40,"color":"#3b82f6"}

    # Pillar 2: Momentum quality (25pts) — RSI position + MACD
    if direction=="long":
        mom = 25 if at_dip else (20 if at_pullback else (15 if at_breakout else 5))
    else:
        mom = 25 if (funding_ok and at_spike) else (15 if at_breakdown else 5)
...
@login_req
def api_opts_chain():
    """Return full options chain for a given expiry/type in one call."""
    b=get_bot(session["uid"])
    if not b.opts: return jsonify({"error":"Not connected"})
    d=request.json or {}
    opt_type=d.get("type","call")
    expiry=d.get("expiry","")
    price=float(d.get("price",0) or b.price or b.api.price() or 79000)
    if not expiry: return jsonify({"rows":[]})
    prefix="C" if opt_type=="call" else "P"
    atm=round(price/500)*500
    offsets=[-2000,-1500,-1000,-500,0,500,1000,1500,2000]
    rows=[]
    for offset in offsets:
        ...
        try:
            ...
            if td and td.get("success"):
                ...
                iv=float(res.get("mark_iv",0) or 0)
                if mark>0:
                    rows.append({"strike":strike,"mark":round(mark,1),
                        "bid":round(bid,1),"ask":round(ask,1),
                        "iv":round(iv,1),"premium_usd":round(mark*0.001,3),
                        "sym":sym,"found":True,"atm":strike==atm})
                else:
                    rows.append({"strike":strike,"mark":0,"found":False,"sym":sym,"atm":strike==atm})
            else:
                rows.append({"strike":strike,"mark":0,"found":False,"sym":sym,"atm":strike==atm})
        except Exception as e:
            rows.append({"strike":strike,"mark":0,"found":False,"sym":sym,"atm":strike==atm})
    rows.sort(key=lambda x:x["strike"])
    return jsonify({"rows":rows,"atm":atm,"price":round(price,1),"expiry":expiry,"type":opt_type})
