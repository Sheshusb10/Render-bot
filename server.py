"""
ALPHA BOT v8 — Delta Exchange India
All-in-one: trading engine + multi-user auth + self-update + dashboard
Single file. No external dependencies beyond requirements.txt
"""
import os,time,hmac,hashlib,json,math,logging,threading,requests,secrets,subprocess,sys
...
class C:
    ...
    OPT_TP  = 0.70; OPT_STOP=0.15; OPT_LOCK=0.64
    OPT_MAX = 0.15; OPT_EXP_BUF=180
    HALT    = 0.08; PAUSE=0.03; COOLDOWN=30
    CIRC_N  = 3;   CIRC_MIN=120; MIN_HOLD=15
    CONF_TRADE=62; STATE="/tmp/ab.json"
    DEPLOY_TOKEN=os.getenv("DEPLOY_TOKEN","alphabot2025deploy")
    GITHUB_RAW="https://raw.githubusercontent.com/Sheshusb10/Render-bot/main/server.py"
    ...
    def login(self,username,password):
        with self._lk:
            for uid,u in self.db["users"].items():
                if u["username"].lower()==username.lower() and u["pw_hash"]==self._hash(password): return True,uid
            return False,None
...
def get_bot(uid):
    if uid not in bots:
        b=Bot(); b._sf=f"/tmp/ab_{uid}.json"; bots[uid]=b
    return bots[uid]
...
class DeltaAPI:
    ...
    def get(self,path,p=None):
        ...
        try: r=self.sess.get(f"{C.BASE}{path}{qs}",headers=self._sign("GET",path,qs),timeout=10); return r.json()
        except Exception as e: log.warning(f"GET {path}: {e}"); return None
    def post(self,path,body):
        b=json.dumps(body)
        try: r=self.sess.post(f"{C.BASE}{path}",headers=self._sign("POST",path,"",b),data=b,timeout=10); return r.json()
    ...
    def should_exit(self,sym,cur,entry,opened_at):
        ...
        if exp:
            try:
                ...
                if now>=exp_dt-timedelta(minutes=C.OPT_EXP_BUF):
        ...
        # "Peak was +5% → lock at +3.2%" (5 * 0.64 = 3.2)
        if peak_pct>0:
            lock=peak_pct*C.OPT_LOCK
            if pct<lock: return {"exit":True,"reason":f"floor: peak+{peak_pct*100:.1f}% lock+{lock*100:.1f}% now+{pct*100:.1f}%","pct":pct}
        # Min hold 5 min
...
class Bot:
    ...
    def _sync_wallet(self):
        ...
        if self.start_cap>0:
            ...
            if loss>=C.HALT and not self.halted:
    ...
    def _sync_pos(self):
        for p in self.api.btcusd_pos():
            ...
            if pid not in self._stops and entry>0:
                sp=entry*(1-C.STOP if side=="long" else 1+C.STOP); tp=entry*(1+C.TP if side=="long" else 1-C.TP)
    ...
    def _on_close(self,won,pnl,entry,exit_p,lots,reason):
        ...
        if won: self._consec=0; self.wins+=1
            ...
            if self._consec>=C.CIRC_N:
                ...
                self.emit("WARN",f"CIRCUIT: {self._consec} losses — pause {C.CIRC_MIN}min")
    ...
    def _pos_disp(self,positions=None):
        ...
        for p in positions:
            ...
            out.append({"sym":C.SYMBOL,"side":side,"lots":abs(sz),"entry":round(entry,1),"mark":round(mark,1),"upnl":round(upnl,3),"pct":round(pct,2),"stop":round(entry*(1-C.STOP if side=="long" else 1+C.STOP),1),"tp":round(entry*(1+C.TP if side=="long" else 1-C.TP),1)})
        return out
    ...
    def _opts_disp(self):
        ...
        for p in self.api.opt_pos():
            ...
            if sz<=0: continue
            pct=(mark-entry)/entry*100 if entry>0 else 0; peak=self.opts._peak.get(sym,entry)
            out.append({"sym":sym,"lots":int(sz),"entry":round(entry,4),"mark":round(mark,4),"upnl":round(float(p.get("unrealized_pnl") or 0),3),"pct":round(pct,1),"peak":round(peak,4),"type":"CALL" if sym.startswith("C-") else "PUT"})
        return out
    ...
    def _get_candles(self):
        ...
        c5m=d5m if len(d5m)>=55 else b5m; c1m=d1m if len(d1m)>=20 else b1m
        bnc_lead="neutral"
        ...
        log.info(f"Candles: d5m={len(d5m)} b5m={len(b5m)} using={'delta' if d5m else 'binance'}")
    ...
    def scan(self):
        ...
        if len(candles.get("5m",[]))<30: self.status=f"Fetching data…"; return
        self.price=candles["5m"][-1]["close"]
        ...
        self.emit("INFO",f"#{self.scan_n} ${self.price:,.0f}|{regime}|ADX={best['adx']} BW={best['bw']}|L={rl['total']} S={rs['total']}|→{strat}")
        now=datetime.now(timezone.utc)
        if self._circuit and now<self._circuit:
            left=int((self._circuit-now).seconds/60); self.status=f"Circuit: {left}m"; return
        ...
        if self.opts_mode and self.opts:
            ...
            if strat=="STRADDLE" and best["bw"]<1.5:
                ...
                if st.get("found") and st["total_premium_usd"]<=self.capital*C.OPT_MAX*2:
                    ...
                    if cp and pp:
                        ...
                        self.status=f"STRADDLE ${st['total_premium_usd']:.2f}"
            ...
            opt=self.opts.find(opt_type,self.price,conf>=78)
            ...
            if r.get("success"):
                ...
                self.status=f"OPT {opt_type.upper()} {opt['symbol']} ${opt['premium_usd']:.2f}"
                self.emit("TRADE",self.status); self.total_tr+=1
        ...
        margin=self.price*C.LOT/C.LEV; lots=max(1,min(int(max(self.capital*C.RISK,margin)/margin),max(1,int(self.capital*.10/margin))))
    ...
    def state(self):
        ...
        return {"connected":self.connected,"running":self.running,"halted":self.halted,"halt_msg":self.halt_msg,"status":self.status,"price":round(self.price,1),"regime":cf.get("regime","—"),"strategy":cf.get("strategy","—"),"vol_regime":cf.get("volatility_regime","—"),"adx":cf.get("adx",0),"bw":cf.get("bw",0),"atr_pct":cf.get("atr_pct",0),"conf_long":sum(v["score"] for v in pls.values()) if pls else 0,"pillars":{k:{"s":v["score"],"m":v["max"],"d":v.get("detail","")} for k,v in pls.items()},"capital":round(self.capital,2),"start_cap":round(sc,2),"pnl_pct":round(pnl,2),"win_rate":round(wr,1),"total_trades":self.total_tr,"wins":self.wins,"next_scan":self.next_scan,"scan_n":self.scan_n,"opts_mode":self.opts_mode,"open_pos":self._pos_disp(),"opts_pos":self._opts_disp(),"trades":list(reversed(self.trades[-50:])),"logs":list(reversed(self.logs[-80:])),"guardrails":{"Stop loss":f"{C.STOP*100:.1f}%","Take profit":f"{C.TP*100:.1f}%","Opt TP":f"+{C.OPT_TP*100:.0f}%","Opt SL":f"-{C.OPT_STOP*100:.0f}%","Opt floor lock":f"64% of peak profit","Monthly halt":f"-{C.HALT*100:.0f}%","Daily pause":f"-{C.PAUSE*100:.0f}%","Cooldown":f"{C.COOLDOWN}min","Circuit":f"{C.CIRC_N} losses={C.CIRC_MIN}min","Min hold":f"{C.MIN_HOLD}min"}}
...
# Auto-connect from env vars if set (for Render/legacy)
if C.KEY and C.SECRET:
    threading.Thread(target=lambda:get_bot("env").connect(C.KEY,C.SECRET),daemon=True).start()

@app.after_request
def _h(r):
    r.headers.update({"Access-Control-Allow-Origin":request.headers.get("Origin","*"),"Access-Control-Allow-Methods":"GET,POST,OPTIONS","Access-Control-Allow-Headers":"Content-Type","Access-Control-Allow-Credentials":"true"})
    return r
...
def auth_setup():
    ...
    d=request.json or {}
    if d.get("setup_key")!=os.getenv("SETUP_KEY","alphabotsetup"): return jsonify({"error":"Wrong setup key"}),403
...
def api_manual():
    ...
    if r.get("success"):
        ...
        b.trades.append({"time":datetime.now(timezone.utc).isoformat(),"side":dirn,"entry":round(p,1),"exit":None,"lots":lots,"pnl":None,"pct":None,"reason":"manual","won":None,"pid":str(C.PID),"sym":C.SYMBOL})
        b.save()
...
def api_self_update():
    ...
    def do_update():
        try:
            ...
            r=requests.get(C.GITHUB_RAW,timeout=30)
...
def admin_users():
    ...
    for uid,u in users.items():
        b=bots.get(uid); u["bot_running"]=b.running if b else False; u["balance"]=b.capital if b else 0; u["trades"]=b.total_tr if b else 0
