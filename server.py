"""
ALPHA BOT — Multi-User Edition
5 isolated users, per-user settings, HTTPS-ready
Each user: own Delta API keys, own stop/TP/settings, own bot instance
"""
import os,time,hmac,hashlib,json,math,logging,threading,secrets,requests
...
# ═══ GLOBAL CONFIG ═══════════════════════════════════════════════
MAX_USERS   = 5
SECRET_KEY  = os.getenv("BOT_SECRET", secrets.token_hex(32))
...
class UserManager:
    ...
    def _save(self):
        try: json.dump(self.data,open(USERS_FILE,"w"),indent=2)
        except Exception as e: log.warning(f"UserManager save: {e}")

    def _hash(self,pw): return hashlib.pbkdf2_hmac("sha256",pw.encode(),"alphabot".encode(),200000).hex()

    def gen_invites(self, n=5):
        with self._lock:
            codes = [secrets.token_urlsafe(12) for _ in range(n)]
            self.data["invites"].extend(codes)
            self._save()
            return codes

    def register(self, invite, username, password):
        with self._lock:
            if invite not in self.data["invites"]:
            ...
            self.data["users"][uid] = {
                ...
                "is_admin": len(self.data["users"]) == 0,
            }
            self.data["invites"].remove(invite)
            self._save()
            return True, uid
    ...
    def get_user(self, uid): return self.data["users"].get(uid)
    def all_users(self): return {uid:{k:v for k,v in u.items() if k!="pw_hash"} for uid,u in self.data["users"].items()}
    def is_admin(self, uid): u=self.get_user(uid); return u and u.get("is_admin",False)
    def pending_invites(self): return list(self.data["invites"])
...
class UserCfg:
    DEFAULTS = dict(
        stop_pct=0.025, tp_pct=0.030, halt_pct=0.08, pause_pct=0.03,
        conf_trade=62,  risk_pct=0.015, opts_mode=False, max_daily=10,
        opt_tp=0.80,    opt_sl=0.50,    opt_floor=0.60,  cooldown=30,
        circuit_n=3,    circuit_min=120, min_hold=15,     adx_trend=22,
    )
    def __init__(self, **kw):
        for k,v in self.DEFAULTS.items(): setattr(self,k,kw.get(k,v))
    def to_dict(self): return {k:getattr(self,k) for k in self.DEFAULTS}
    @classmethod
    def from_dict(cls,d): return cls(**{k:d.get(k,v) for k,v in cls.DEFAULTS.items()})

# ═══ CONSTANTS (shared, read-only) ════════════════════════════════
class C:
...
def atr_tp_sl(atr_pct,base_tp,base_sl):
    if atr_pct<=0: return base_tp,base_sl
    if atr_pct<0.30: return max(atr_pct*1.5/100,0.010),max(atr_pct*1.0/100,0.008)
...
class ConfEngine:
    ...
    def score(self,candles,direction,hour):
        c5m=candles.get("5m",[]); c1m=candles.get("1m",[]); c15m=candles.get("15m",[])
        if len(c5m)<30: return {"total":0,"veto":f"need_30_candles_have_{len(c5m)}","regime":"UNKNOWN","strategy":"WAIT","pillars":{},"vol_regime":"UNKNOWN","adx":0,"bw":0,"atr_pct":0}
        ...
        cl15=[c["close"] for c in c15m]; hi15=[c["high"] for c in c15m]; lo15=[c["low"] for c in c15m]
        price=cl5[-1]
        p={}
        # Regime pillar
        adx_v,pdi,ndi=adx_calc(hi5,lo5,cl5)
        if len(cl5)>=55: e8=ema(cl5,8)[-1]; e21=ema(cl5,21)[-1]; e55=ema(cl5,55)[-1]
        else: e8=ema(cl5,8)[-1]; e21=ema(cl5,21)[-1]; e55=cl5[0]
        ...
        if direction=="long":
            ...
            elif r5<35: rs2,rd2=12,"Oversold bounce"
            elif r5<=65: rs2,rd2=7,"Mid-range"
            else: rs2,rd2=3,"Overbought"
        else:
            if 45<=r5<=65 and r1<r5: rs2,rd2=15,"Distribution+falling"
            elif r5>65: rs2,rd2=12,"Overbought rejection"
        ...
        ln,sg,hist=macd(cl5)
        if direction=="long":
            if hist>0 and ln>sg: ms3,md3=15,"MACD bullish"
            elif hist>0: ms3,md3=8,"Hist positive"
            else: ms3,md3=2,"MACD bearish"
        else:
            if hist<0 and ln<sg: ms3,md3=15,"MACD bearish"
            elif hist<0: ms3,md3=8,"Hist negative"
            else: ms3,md3=2,"MACD bullish"
        ...
        elif bw<0.5: vs,vd=8,"Squeeze-ready"
        ...
        if len(vo5)>=21:
            ...
            if cur<avg5*0.1: p["Volume"]={"score":0,"max":10,"detail":"vol trap","veto":"low_vol"}
            elif cur>avg5*2: p["Volume"]={"score":10,"max":10,"detail":"Vol spike"}
        ...
        elif hour in prime: p["Session"]={"score":5,"max":5,"detail":"prime session"}
        ...
        elif regime=="SIDEWAYS" and vol_regime=="LOW": strategy="STRADDLE"
...
conf_engine = ConfEngine()

# ═══ BOT (per-user instance) ══════════════════════════════════════
class Bot:
    def __init__(self,uid,cfg=None):
        self.uid=uid; self.cfg=cfg or UserCfg()
        self.api=DeltaAPI(); self.running=False; self.connected=False
        self.opts_mode=False; self.capital=0.0; self.start_cap=0.0
        self.day_start=0.0; self.halted=False; self.halt_msg=""
    ...
    @property
    def state_file(self): return f"{STATE_DIR}/ab_{self.uid}.json"

    def emit(self,level,msg):
        e={"t":datetime.now(timezone.utc).strftime("%H:%M:%S"),"l":level,"m":msg}
        self.logs.append(e)
        if len(self.logs)>300: self.logs.pop(0)
        getattr(log,{"INFO":"info","WARN":"warning","ERROR":"error","TRADE":"info"}.get(level,"info"))(f"[{self.uid[:6]}] {msg}")
    ...
    def save(self):
        try:
            json.dump({"start_cap":self.start_cap,"day_start":self.day_start,"halted":self.halted,
                ...
                "consec":self._consec,"daily_trades":self._daily_trades,"daily_date":self._daily_date,
                ...
                "cfg":self.cfg.to_dict()},open(self.state_file,"w"))
        except Exception as e: log.warning(f"save {self.uid}: {e}")
    ...
    def load(self):
        try:
            ...
            self._consec=int(s.get("consec",0)); self._daily_trades=int(s.get("daily_trades",0))
            ...
            if s.get("cfg"): self.cfg=UserCfg.from_dict(s["cfg"])
    ...
    def connect(self,key,secret):
        ...
        self.emit("INFO",f"Connected ${bal:.2f} | Halt<${self.start_cap*(1-self.cfg.halt_pct):.2f}")
        self._sync_pos()
        if not self.running: self.start()
        return {"success":True,"balance":bal}

    def disconnect(self):
        self.stop(); self.connected=False; self.api.set("","")
        self.emit("INFO","Disconnected")

    def update_cfg(self,d):
        for k,v in d.items():
            if hasattr(self.cfg,k): setattr(self.cfg,k,v)
        self.save()
        self.emit("INFO",f"Settings updated: {list(d.keys())}")
    ...
    def _sync_wallet(self):
        ...
        if self.start_cap>0:
            ...
            if loss>=self.cfg.halt_pct and not self.halted:
        ...
        for p in self.api.btcusd_pos():
            ...
            if not any(pid_int(t.get("pid",0))==pid and t.get("exit") is None for t in self.trades):
                ...
                if pid not in self._stops:
                    sp=entry*(1-self.cfg.stop_pct if side=="long" else 1+self.cfg.stop_pct)
                    tp=entry*(1+self.cfg.tp_pct   if side=="long" else 1-self.cfg.tp_pct)
                    r=self.api.bracket("sell" if side=="long" else "buy",lots,sp,tp)
                    if r.get("success"): self._stops.add(pid)
    ...
    def _check_exits(self,positions):
        ...
        for p in positions:
            ...
            hold_min=(now-opened).seconds//60 if opened else self.cfg.min_hold+1
            if hold_min<self.cfg.min_hold: continue
            reason=None
            if pct<=-self.cfg.stop_pct: reason="stop"
            elif pct>=self.cfg.tp_pct: reason="tp"
            if reason:
                r=self.api.close(sz,pid)
                if r.get("success"):
                    pnl=round(entry*lots*C.LOT*pct,4); won=pct>0
                    self.emit("TRADE",f"{'✅TP' if won else '❌SL'} {side.upper()} {lots}L ${entry:.0f}→${self.price:.0f} P&L ${pnl:+.4f} held={hold_min}m")
                    self._on_close(won,pnl,side,entry,self.price,lots,reason)
    ...
    def _on_close(self,won,pnl,side,entry,exit_p,lots,reason):
        ...
        if won: self._consec=0; self.wins+=1
            ...
            self._consec+=1
            if self._consec>=self.cfg.circuit_n:
                self._circuit=now+timedelta(minutes=self.cfg.circuit_min)
                self.emit("WARN",f"CIRCUIT BREAKER: {self._consec} losses — pause {self.cfg.circuit_min}min")
        for t in reversed(self.trades):
            if t.get("exit") is None and t.get("entry")==round(entry,1):
                t.update({"exit":round(exit_p,1),"pnl":pnl,"pct":round(pct/100 if abs(pnl)<1 else pnl/max(entry*lots*C.LOT,0.001)*100,2),"won":won,"reason":reason})
                break
        self.save()
    ...
    def _pos_display(self,positions=None):
        ...
        for p in positions:
            ...
            out.append({"sym":C.SYMBOL,"side":side,"lots":abs(sz),"entry":round(entry,1),"mark":round(mark,1),"upnl":round(upnl,3),"pct":round(pct,2),"stop":round(entry*(1-self.cfg.stop_pct if side=="long" else 1+self.cfg.stop_pct),1),"tp":round(entry*(1+self.cfg.tp_pct if side=="long" else 1-self.cfg.tp_pct),1)})
        return out
    ...
    def scan(self):
        ...
        # Multi-source candles
        sess=requests.Session()
        def binance(interval,limit=100):
            try:
                r=sess.get("https://api.binance.com/api/v3/klines",params={"symbol":"BTCUSDT","interval":interval,"limit":limit},timeout=8)
                if r.status_code!=200: return []
                return [{"close":float(c[4]),"high":float(c[2]),"low":float(c[3]),"volume":float(c[5])} for c in r.json()]
            except: return []
        d5m=_parse(self.api.candles("5m",100)); b5m=binance("5m")
        d1m=_parse(self.api.candles("1m",100)); b1m=binance("1m")
        ...
        self._check_exits(real); self._sync_pos()
        hour=datetime.now(timezone.utc).hour
        rl=conf_engine.score(candles,"long",hour); rs=conf_engine.score(candles,"short",hour)
        ...
        self.emit("INFO",f"#{self.scan_n} ${self.price:,.0f}|{regime}|ADX={best['adx']}|L={rl['total']}{'✗'+lv if lv else ''} S={rs['total']}{'✗'+sv if sv else ''}|→{strat}")
        # Guards
        now=datetime.now(timezone.utc)
        ...
        if self._last_close and (now-self._last_close).seconds<self.cfg.cooldown*60:
            gap=self.cfg.cooldown-(now-self._last_close).seconds//60; self.status=f"Cooldown: {gap}m"; return
        if self.day_start>0 and (self.capital-self.day_start)/self.day_start<=-self.cfg.pause_pct:
            self.status="Paused — daily limit"; return
        if self._daily_trades>=self.cfg.max_daily:
            self.status=f"Daily trade limit {self.cfg.max_daily} reached"; return
        if len(real)>=1:
        ...
        if strat=="WAIT" or best["total"]<self.cfg.conf_trade:
        ...
        lots=max(1,min(int(max(self.capital*self.cfg.risk_pct,margin)/margin),max(1,int(self.capital*.10/margin))))
        side="buy" if direction=="long" else "sell"
        r=self.api.order(side,lots)
        if not r.get("success"): self.emit("ERROR",f"Order failed: {r.get('error',r.get('message','?'))}"); return
        dyn_tp,dyn_sl=atr_tp_sl(best.get("atr_pct",0),self.cfg.tp_pct,self.cfg.stop_pct)
        ...
        self.emit("TRADE",f"{self.status} | {strat} | SL=${sp:.0f} TP=${tp:.0f}")
        self.total_tr+=1
    ...
    def _loop(self):
        while self.running:
            try: self.scan()
            except Exception as e: log.error(f"scan [{self.uid}]: {e}",exc_info=True); self.status=f"Error: {e}"
            time.sleep(C.SCAN)
    ...
    def state(self):
        ...
        return {"connected":self.connected,"running":self.running,"halted":self.halted,"halt_msg":self.halt_msg,
            ...
            "daily_trades":self._daily_trades,"next_scan":self.next_scan,"scan_n":self.scan_n,
            "opts_mode":self.opts_mode,"open_pos":self._pos_display(),
            "trades":list(reversed(self.trades[-50:])),"logs":list(reversed(self.logs[-80:])),
            "cfg":self.cfg.to_dict(),
            "guardrails":{"Stop loss":f"{self.cfg.stop_pct*100:.1f}%","Take profit":f"{self.cfg.tp_pct*100:.1f}%",
                "Monthly halt":f"-{self.cfg.halt_pct*100:.0f}%","Daily pause":f"-{self.cfg.pause_pct*100:.0f}%",
                "Max daily trades":str(self.cfg.max_daily),"Confidence threshold":str(self.cfg.conf_trade),
                "Cooldown":f"{self.cfg.cooldown}min","Circuit":f"{self.cfg.circuit_n} losses={self.cfg.circuit_min}min",
                "Min hold":f"{self.cfg.min_hold}min","Risk per trade":f"{self.cfg.risk_pct*100:.1f}%"}}
...
def get_bot(uid):
    if uid not in bots: bots[uid]=Bot(uid)
    return bots[uid]
...
def auth_login():
    ...
    if not ok: return jsonify({"success":False,"message":"Invalid username or password"}),401
    session["uid"]=uid; session.permanent=True
    user=um.get_user(uid)
    return jsonify({"success":True,"username":user["username"],"is_admin":user.get("is_admin",False)})

@app.route("/auth/logout", methods=["POST"])
def auth_logout():
    uid=session.pop("uid",None)
    if uid and uid in bots: bots[uid].disconnect()
...
def auth_me():
    ...
    user=um.get_user(uid)
    if not user: return jsonify({"logged_in":False}),200
    return jsonify({"logged_in":True,"username":user["username"],"is_admin":user.get("is_admin",False)})

# ── Bot API (login required) ──────────────────────────────────────
...
@login_required
def api_disconnect():
    get_bot(session["uid"]).disconnect()
    return jsonify({"success":True})
...
@login_required
def api_run():
    bot=get_bot(session["uid"]); threading.Thread(target=bot.scan,daemon=True).start()
    return jsonify({"success":True})
...
def api_settings():
    d=request.json or {}
    allowed={"stop_pct","tp_pct","halt_pct","pause_pct","conf_trade","risk_pct","max_daily","cooldown"}
    updates={}
    for k,v in d.items():
        ...
        try:
            ...
            limits={"stop_pct":(0.005,0.10),"tp_pct":(0.005,0.15),"halt_pct":(0.03,0.30),
                "pause_pct":(0.01,0.15),"conf_trade":(40,95),"risk_pct":(0.005,0.05),
                "max_daily":(1,50),"cooldown":(5,120)}
            mn,mx=limits.get(k,(0,9999))
            updates[k]=max(mn,min(mx,fv))
        except: pass
    get_bot(session["uid"]).update_cfg(updates)
    return jsonify({"success":True,"cfg":get_bot(session["uid"]).cfg.to_dict()})
...
def api_manual():
    ...
    bot=get_bot(session["uid"]); p=bot.price or bot.api.price(); lots=max(1,int(d.get("lots",1)))
    r=bot.api.order("buy" if dirn=="long" else "sell",lots)
    if r.get("success"):
        sp=p*(1-bot.cfg.stop_pct if dirn=="long" else 1+bot.cfg.stop_pct)
        tp=p*(1+bot.cfg.tp_pct   if dirn=="long" else 1-bot.cfg.tp_pct)
        bot.api.bracket("sell" if dirn=="long" else "buy",lots,sp,tp)
        bot.emit("TRADE",f"MANUAL {dirn.upper()} {lots}L @ ${p:,.0f}")
        bot.trades.append({"time":datetime.now(timezone.utc).isoformat(),"side":dirn,"entry":round(p,1),"exit":None,"lots":lots,"pnl":None,"pct":None,"reason":"manual","won":None,"pid":str(C.PID),"sym":C.SYMBOL})
...
@login_required
def api_close_all():
    bot=get_bot(session["uid"]); n=0
    for p in bot.api.btcusd_pos():
        sz=float(p.get("size",0) or 0); r=bot.api.close(sz,p.get("product_id",C.PID))
...
@login_required
def api_opts():
    d=request.json or {}; bot=get_bot(session["uid"])
    bot.opts_mode=bool(d.get("enabled",not bot.opts_mode))
    bot.emit("INFO","Options mode ON" if bot.opts_mode else "Options mode OFF")
    return jsonify({"success":True,"opts_mode":bot.opts_mode})
...
def admin_users():
    ...
    for uid,u in users.items():
        b=bots.get(uid); result[uid]={**u,"bot_running":b.running if b else False,"balance":b.capital if b else 0,"trades":b.total_tr if b else 0}
    return jsonify({"users":result,"invites":um.pending_invites(),"max_users":MAX_USERS})

@app.route("/api/admin/invite", methods=["POST"])
@admin_required
def admin_invite():
    codes=um.gen_invites(1)
    return jsonify({"success":True,"code":codes[0]})

@app.route("/api/admin/setup", methods=["POST"])
def admin_setup():
    """One-time setup: creates first admin user. Disabled after first user exists."""
    if um.data["users"]: return jsonify({"error":"Setup already done"}),403
    d=request.json or {}
    if d.get("setup_key") != os.getenv("SETUP_KEY","alphabotsetup"):
        return jsonify({"error":"Wrong setup key"}),403
    codes=um.gen_invites(1)
    ok,result=um.register(codes[0],d.get("username","admin"),d.get("password",""))
...
if __name__ == "__main__":
    ...
    if "--setup" in sys.argv:
        codes = um.gen_invites(5)
        print("\n5 invite codes generated:")
        for cc in codes: print(f"  {cc}")
        print("\nShare one code per user. Each code works once.")
        sys.exit()
