...
def score(candles,direction,hour):
    ...
    if len(vo)>=21:
        ...
        if cur<avg5*0.1:   p["Volume"]={"score":0, "max":10,"detail":"Volume trap"}
        ...
        else:              p["Volume"]={"score":2, "max":10,"detail":"Low"}
...
class Bot:
    ...
    def scan(self):
        ...
        if self.opts_mode and self.opts:
            ...
            if r.get("success"):
                ...
                self.trades.append({"time":now.isoformat(),"side":opt_type,"entry":round(opt["mark"],4),
                    "exit":None,"lots":1,"pnl":None,"pct":None,"reason":strat.lower(),
        ...
        # Lower threshold when 1H and 5m agree — higher conviction
        if h1_trend=="bull" and regime in ("BULL","STRONG_BULL"):   conf_needed=50
        elif h1_trend=="bear" and regime in ("BEAR","STRONG_BEAR"): conf_needed=50
        elif regime in ("STRONG_BULL","STRONG_BEAR"):               conf_needed=55
        else:                                                         conf_needed=C.CONF_TRADE
        if strat=="WAIT" or best["total"]<conf_needed:
            self.status=f"Watching {regime} 1H={h1_trend} score={best['total']} {best.get('veto','')}"; return
        ...
        self.status=f"{'★' if best.get('div') else ''}{direction.upper()} {lots}L @ ${self.price:,.0f} conf={best['total']} SL=${sp:.0f} TP=${tp:.0f}"
