...
"losses": 0,
"profit": 0.0,
"max_trades": 8, "target_pct": 8.0, "stop_pct": -12.0, "min_confidence": 52, # max scalper trades per day
# take profit at +8%
# stop loss at -12%
# min confidence to scalp
},
...
url = f"https://api.binance.us/api/v3/klines?symbol={sym}&interval={interval}&limit={limi
try:
...
try:
r = requests.get(
f"https://api.binance.com/api/v3/klines?symbol={sym}&interval={interval}&limit={l
timeout=10)
if r.status_code == 200:
data = r.json()
if isinstance(data, list) and len(data) > 0:
...
return {
"spike": "fake_pump": "fake_dump": "volume_trend": last_vol > avg_vol * 3,
chg > 1.5 and last_vol < avg_vol * 0.7,
chg < -1.5 and last_vol < avg_vol * 0.7,
("RISING" if last_vol > avg_vol*1.2
...
chg_1h = ((c1h[-1]["close"]-c1h[-2]["close"])/c1h[-2]["close"]*100) if len(c1h)>=2 chg_4h = ((c4h[-1]["close"]-c4h[-2]["close"])/c4h[-2]["close"]*100) if len(c4h)>=2 chg_1d = ((c1d[-1]["close"]-c1d[-2]["close"])/c1d[-2]["close"]*100) if c1d and len(c1d)>=
# Market regime
...
if abs(chg_1h) > 8.0:
blog(f"[{asset}] Volatility spike {chg_1h:.1f}% — skipping", "warning")
return {
...
bull += 20
reasons.append(f" if rsi_1h < 20:
bull += 15
Extreme oversold RSI {rsi_1h} — bounce signal")
...
# ── Volume / Whale ────────────────────────────────────────────
if vol["fake_pump"]: bear += 25; reasons.append(" FAKE PUMP detected")
if vol["fake_dump"]: bull += 25; reasons.append(" FAKE DUMP detected")
...
setup_key = f"{regime}_{direction.replace('BUY_','')}" if direction != "NO TRADE" else No
if setup_key:
...
avg_range = sum(c["high"]-c["low"] for c in c1h[-20:]) / 20 if len(c1h) >= 20 else re
...
direction = "NO TRADE"
reasons.append(f"PUT vetoed — {regime} requires {thresholds['PUT']}% bear (got {bear_
if direction == "BUY_CALL" and bull_pct < thresholds["CALL"]:
direction = "NO TRADE"
reasons.append(f"CALL vetoed — {regime} requires {thresholds['CALL']}% bull (got {bul
...
trail["peak"] = pnl_pct
# Progressive floors
floors = [(50,45),(30,25),(25,22),(20,17),(15,12),(12,10),(10,8),(8,6),(5,3),
for threshold, floor in floors:
if pnl_pct >= threshold:
trail["floor"] = floor
blog(f"[{sym}] Trail floor +{floor}% (peak +{pnl_pct:.1f}%)", "suc
break
...
blog(f"[{sym}] ✓ Closed automatically | PnL:{pnl_pct:.1f}%", "suc
...
blog(f" WIN +{pnl_pct:.1f}% | Buffer: +${abs(realized):.3f}
else:
s["losses"] += 1
...
blog(f" LOSS {pnl_pct:.1f}% | Absorbed by buffer | Buff
else:
remaining = loss_amt - s["profit_buffer"]
s["profit_buffer"] = 0
blog(f" LOSS {pnl_pct:.1f}% | Capital hit: -${remaining
s["daily_pnl"] += upnl
...
bot_state["setup_memory"][setup_key] = {"wins": 0, if upnl > 0:
...
blog(f"[{asset}] High conviction {conf}% — allowing despite correlation",
...
blog(f"[{asset}] Selected: {product['symbol']} | Vol={vol_24h:.0f} OI={oi_val:.0f}", "inf
...
if resp.get("error"):
err = resp["error"]
blog(f"[{asset}] Failed: {err.get('code','err')} — {err.get('context','')}", "err
return False
...
if not delta_ok:
blog(f" Delta Exchange: {delta_status} — pausing trading", "error")
return
check_ip()
...
if result.get("error"):
return jsonify({"ok":False,"error":result["error"].get("code","Auth failed")}),40
s = bot_state["stats"]
...
s["starting_balance"] = s["current_balance"] = s["peak_balance"] = s["profit_floor"] = round(bal*0.90,4)
bal
check_ip()
blog(f"Connected {region} | Balance:{s['current_balance']:.2f} | IP:{_ip_state['curre
